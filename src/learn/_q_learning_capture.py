...
#     def __init__(self, agent, env, memsize=1000):
#         """
#         Q-learning object to learn capture chess
#         Args:
#             agent: The agent playing the chess game as white
#             env: The environment including the python-chess board
#             memsize: maximum amount of games to retain in-memory
...
#         self.optimizer = optim.SGD(self.agent.parameters(), lr=0.01)

#     def learn(self, iters=100, c=10):
#         """
#         Run the Q-learning algorithm. Play greedy on the final iteration.
#         Args:
#             iters: int
...
#                 game count, determines epsilon (exploration rate)
#             greedy: bool
#                 if True, no exploration is done
#             maxiter: int
#                 Maximum number of steps per game

#         Returns:
#             board: chess.Board
#                 Final state of the chess board
#         """
#         episode_end = False
#         turncount = 0
...
#         while not episode_end:
#             eps = max(0.05, 1 / (1 + (k / 250))) if not greedy else 0.
#             state = torch.from_numpy(self.env.layer_board).unsqueeze(0).float()
#             explore = np.random.uniform(0, 1) < eps  # determine whether to explore
#             if explore:
#                 move = self.env.get_random_action()
#                 move_from = move.from_square
...
#                 action_space = self.env.project_legal_moves()  # The environment determines which moves are legal
#                 action_values = torch.mul(action_values, torch.tensor(action_space))
...
#             episode_end, reward = self.env.step(move)
#             new_state = torch.from_numpy(self.env.layer_board).unsqueeze(0).float()
#             if len(self.memory) > self.memsize:
#                 self.memory.pop(0)
#                 self.sampling_probs.pop(0)
#             turncount += 1
...
#                 q_values = self.agent.get_action_values(state)
#                 q_values = q_values.view(64, 64)
#                 next_q_values = self.agent.get_action_values(new_state)
#                 next_q_values = next_q_values.view(64, 64)
